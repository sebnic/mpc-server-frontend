  ...
  <div class="container">
    ...
    <div id="testSection" class="section" style="display: none;">
      ...
      <div class="test-card llm-card">
        ...
        <button class="btn btn-primary" id="llmInitBtn">Initialize LLM (~1GB download)</button>
...
document.getElementById('llmInitBtn')!.addEventListener('click', async () => {
  try {
    log('Initializing LLM... This may take a while (downloading ~1GB model)');
    ...
    const result = await client!.callTool('llm_initialize');
    const text = result.content[0].type === 'text' ? result.content[0].text : '';
